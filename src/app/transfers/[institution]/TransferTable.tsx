...
export default function TransferTable({ transfers, title, isInactive = false, openByDefault = false }: TransferTableProps) {
    ...
    const getTransferRowBgColor = (transfer: Transfer, tintred:boolean=false): string => {
        ...
        if (transfer.credit.includes('=')) {
            ...
            const sourceCredits = transfer.sourceCredits;
                    ...
                    {transfers.map(transfer => (
                        <tr key={transfer.id} className={`${getTransferRowBgColor(transfer, isInactive)}`}> 
                            <td className="px-6 py-2">
                                <Link
                          href={`/courses/${transfer.subject.toLowerCase()}-${transfer.courseNumber.toLowerCase()}`}
                          className="text-black hover:text-[#f15a22] underline"
                          target="_blank"
                          prefetch={false}
                        >
                          <span className="font-medium">{`${transfer.subject} ${transfer.courseNumber}`}</span>
                          {transfer.sourceTitle && (
                            <span className="text-gray-500 hover:text-[#f15a22]">{`: ${transfer.sourceTitle}`}</span>
                          )}
                        </Link>
                            </td>
                            ...
                            <td className="px-6 py-2 text-sm text-gray-500">
                                <div>
                                    {formatDate(transfer.effectiveStart)}
                                </div>
                                {transfer.effectiveEnd && (
                                    <div className={`text-xs ${isInactive ? 'text-red-600' : ''}`}>
                                        to {formatDate(transfer.effectiveEnd)}
                                    </div>
                                )}
                                {!transfer.effectiveEnd && (
